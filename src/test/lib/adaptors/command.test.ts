...
suite('CommandAdaptor', () => {
	test('it opens editable diff above with editable-original options', async () => {
		...
		const commands = { executeCommand: async (...args: any[]) => { capturedArgs = args; } } as any;
		const commandAdaptor = new CommandAdaptor(commands, vscode.Uri.parse, console);
	...
	test('it falls back to default group when directional placement is not accepted', async () => {
		...
		const commands = { executeCommand: async (...args: unknown[]) => {
