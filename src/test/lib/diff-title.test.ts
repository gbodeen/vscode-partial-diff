...
suite('Title of Diff', () => {
	...
	suite('When there is no active normalization rules', () => {
		const normalizationRuleStore = mock(NormalizationRuleStore);
		const diffTitleBuilder = new DiffTitleBuilder(normalizationRuleStore, selectionInfoRegistry);
	...
	suite('When there are active normalization rules', () => {
		const normalizationRuleStore = mockType<NormalizationRuleStore>({ hasActiveRules: true });
		const diffTitleBuilder = new DiffTitleBuilder(normalizationRuleStore, selectionInfoRegistry);

		test('it uses \u007E if the comparison was done with text normalization', async () => {
			const title = diffTitleBuilder.build('TEXT1', 'TEXT2');
			assert.equal(title, 'FILE1 \u007E FILE2');
		});
	});
