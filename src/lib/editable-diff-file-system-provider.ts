 ...
 * Using a dedicated scheme avoids opening extra untitled editors
 * while keeping both diff sides backed by writable resources.
 */
export default class EditableDiffFileSystemProvider implements vscode.FileSystemProvider {
	...
	stat(uri: vscode.Uri): vscode.FileStat {
		...
		if (file) {
			return {
				...
				permissions: file.readOnly ? this.readOnlyPermission : undefined
			};
		}
	...
	readDirectory(uri: vscode.Uri): [string, vscode.FileType][] {
		const dirPath = this.normalizePath(uri);
	...
	writeFile(uri: vscode.Uri, content: Uint8Array, options: { create: boolean; overwrite: boolean }): void {
		...
		const readOnlyPath = this.isReadOnlyPath(path);
		...
		this.files.set(path, {
			...
			mtime: now,
			readOnly: existing ? existing.readOnly : readOnlyPath
		});
	...
	rename(oldUri: vscode.Uri, newUri: vscode.Uri, options: { overwrite: boolean }): void {
		const oldPath = this.normalizePath(oldUri);
		const newPath = this.normalizePath(newUri);
	...
	private get readOnlyPermission(): vscode.FilePermission | undefined {
		const filePermission = (vscode as unknown as { FilePermission?: { Readonly?: number } }).FilePermission;
		return typeof filePermission?.Readonly === 'number'
			? filePermission.Readonly as unknown as vscode.FilePermission
			: undefined;
	}
